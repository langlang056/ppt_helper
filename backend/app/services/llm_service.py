...
**重要格式要求**:
- 使用 Markdown 格式回答
- **数学符号使用 Unicode 字符，不要使用 LaTeX 语法（不要用 $ 符号）**
- 常用数学符号示例：
  * 希腊字母：α β γ δ ε θ λ μ π σ ω Σ Ω
  * 运算符：× ÷ ± ≈ ≠ ≤ ≥ ∝ ∞
  * 上下标：x₁ x₂ x³ x² xⁿ
  * 分数：用 / 表示，如 1/2, a/b
  * 积分求和：∫ ∑ ∏
  * 逻辑符号：∈ ∉ ⊂ ⊆ ∪ ∩ ∧ ∨ ¬
- 例如写 "p(a|t) ∝ p(t|a) × p(t)" 而不是 "$p(a|t) \\propto p(t|a) \\times p(t)$"
- 用反引号包裹重要的数学表达式和变量名，如 `r(x)`, `θ₁`, `P(6|θ₂)`
- 请用清晰、易懂的中文回答"""
